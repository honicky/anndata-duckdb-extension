...
### Fixed
- **CI: unbreak the `linux_amd64_musl` and `linux_arm64` builds.** Both had been failing every day since ~2026-07-29. vcpkg's `hdf5` port enables its `szip` feature by default, which pulls in `libaec`, whose portfile at the `extension-ci-tools` default baseline (`84bab45d`, Dec 2025) downloads the source tarball from `gitlab.dkrz.de`. That host now returns HTTP 429 to GitHub Actions runner IP ranges, so the download failed all four vcpkg retries and aborted `vcpkg install`. Only musl/arm64 were affected because they get no hits from DuckDB's read-only vcpkg binary cache (different toolchain → different ABI hash) and therefore build `libaec` from source; `linux_amd64` restores it from cache and only ever showed as `cancelled` via matrix fail-fast. Upstream vcpkg fixed this by moving `libaec` to the `Deutsches-Klimarechenzentrum/libaec` GitHub mirror, so `vcpkg.json` now pins `builtin-baseline` to vcpkg `2026.07.29` (`9e593bb1`) and the three `_extension_distribution.yml` call sites pass a matching `vcpkg_commit`.
...
### Changed
- Pinned `hdf5` to 1.14.6 via a vcpkg `overrides` entry. The new baseline would otherwise have moved HDF5 to 2.1.1, a major-version jump that was not part of this fix: `CMakeLists.txt` links the 1.14-era `hdf5::hdf5-static` / `hdf5::hdf5-shared` config targets, and local development builds against the system HDF5 1.14.x, so an unpinned CI-only bump would have made CI diverge from local. Lifting the pin should be done deliberately, with its own build verification.
- Because the baseline moved, `openssl`, `curl`, and `zlib` also advance to their `2026.07.29` versions. This invalidates the shared vcpkg binary cache for the archs that were previously hitting it (`linux_amd64`, osx, windows), so those builds now compile their dependencies from source and take a few minutes longer.

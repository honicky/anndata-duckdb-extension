...
- **CI: unbreak the `linux_amd64_musl` and `linux_arm64` builds.** Both had been failing every day since ~2026-07-29. vcpkg's `hdf5` port enables its `szip` feature by default, which pulls in `libaec`, whose portfile at the baseline DuckDB pins (`84bab45d`, Dec 2025) downloads the source tarball from `gitlab.dkrz.de`. That host now returns HTTP 429 to GitHub Actions runner IP ranges, so the download failed all four vcpkg retries and aborted `vcpkg install`. Only musl/arm64 were affected because they get no hits from DuckDB's read-only vcpkg binary cache (different toolchain → different ABI hash) and therefore build `libaec` from source; `linux_amd64` restores it from cache and only ever showed as `cancelled` via matrix fail-fast.
  ...
  Fixed by vendoring a **vcpkg overlay port** at `vcpkg_ports/libaec/` — a verbatim copy of the upstream port that microsoft/vcpkg fixed by moving the source to the `Deutsches-Klimarechenzentrum/libaec` GitHub mirror (libaec 1.1.7) — and declaring it via `vcpkg-configuration.overlay-ports` in `vcpkg.json`. Overlay ports take precedence over the baseline and registries, so this replaces the one broken port and nothing else.

  Note that a plain baseline bump is **not** possible from this repo: `duckdb/scripts/merge_vcpkg_deps.py` regenerates the manifest that CI actually builds from, hardcoding `builtin-baseline` and replacing `overrides` wholesale. It reads only `dependencies` and `vcpkg-configuration` out of our `vcpkg.json`, so `overlay-ports` is the supported hook. The script rewrites the overlay path relative to our manifest's location, which resolves to an absolute path at the repo root in CI.

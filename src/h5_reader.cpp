...
bool H5Reader::IsValidAnnData() {
    ...
    return IsGroupPresent("/obs") && IsGroupPresent("/var") && 
           (IsGroupPresent("/X") || IsDatasetPresent("/", "X"));
}
...
size_t H5Reader::GetVarCount() {
    try {
        ...
        for (const auto &member : members) {
            if (!StringUtil::StartsWith(member, "_") && 
                IsDatasetPresent("/var", member)) {
...
LogicalType H5Reader::H5TypeToDuckDBType(const H5::DataType &h5_type) {
    ...
    switch (type_class) {
        case H5T_INTEGER: {
            ...
            if (size <= 1) return LogicalType::TINYINT;
            if (size <= 2) return LogicalType::SMALLINT;
            if (size <= 4) return LogicalType::INTEGER;
            return LogicalType::BIGINT;
        }
        case H5T_FLOAT: {
            size_t size = h5_type.getSize();
            if (size <= 4) return LogicalType::FLOAT;

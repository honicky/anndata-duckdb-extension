...
void RegisterAnndataTableFunctions(DatabaseInstance &db) {
    ...
    TableFunction obs_func("anndata_scan_obs", {LogicalType::VARCHAR}, AnndataScanner::ObsScan,
                          AnndataScanner::ObsBind, AnndataInitGlobal, AnndataInitLocal);
    obs_func.name = "anndata_scan_obs";
    ExtensionUtil::RegisterFunction(db, obs_func);
    
    // Register anndata_scan_var function
    TableFunction var_func("anndata_scan_var", {LogicalType::VARCHAR}, AnndataScanner::VarScan,
                          AnndataScanner::VarBind, AnndataInitGlobal, AnndataInitLocal);
    var_func.name = "anndata_scan_var";
    ExtensionUtil::RegisterFunction(db, var_func);
    
    // Register anndata_info function (scalar function)
    auto info_func = ScalarFunction("anndata_info", {LogicalType::VARCHAR}, LogicalType::VARCHAR,
                                   [](DataChunk &args, ExpressionState &state, Vector &result) {
                                       auto &path_vec = args.data[0];
                                       UnaryExecutor::Execute<string_t, string_t>(
                                           path_vec, result, args.size(),
                                           [&](string_t path) {
                                               return StringVector::AddString(result, 
                                                   AnndataScanner::GetAnndataInfo(path.GetString()));
                                           });
                                   });

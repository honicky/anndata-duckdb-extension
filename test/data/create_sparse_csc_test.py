#!/usr/bin/env python3
"""Create a test AnnData file with sparse X matrix in CSC format."""
...
# Generate sparse data in CSC format
sparse_data = sp.random(n_obs, n_var, density=density, format='csc', random_state=42)
...
# Save to file
adata.write_h5ad('test_sparse_csc.h5ad')

# Manually add encoding-type attribute to ensure CSC format is properly marked
with h5py.File('test_sparse_csc.h5ad', 'r+') as f:
    if 'X' in f:
        f['X'].attrs['encoding-type'] = 'csc'
        f['X'].attrs['encoding-version'] = '0.1.0'
        f['X'].attrs['shape'] = [n_obs, n_var]

print(f"Created test_sparse_csc.h5ad with CSC sparse matrix ({n_obs} obs x {n_var} var)")
print(f"Sparsity: {1 - sparse_data.nnz / (n_obs * n_var):.1%}")
print(f"Non-zero elements: {sparse_data.nnz}")
print(f"Format: {sparse_data.format.upper()}")
